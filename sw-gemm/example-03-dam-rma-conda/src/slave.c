...
__thread_local crts_rply_t rma_rply = 0;
...
void big_kernal(SWARGS* args)
{
    ...
    // 优化方向：----->>>>>> DMA + RMA + Cannon卡农算法

    int A_buffer_size = 36 * 1024 / 8;
    int B_buffer_size = 36 * 1024 / 8;
    double* A_dma_buffer = (double*)ldm_malloc(A_buffer_size * sizeof(double)); // 36KB
    double* A_rma_buffer = (double*)ldm_malloc(A_buffer_size * sizeof(double)); // 36KB
    double* B_dma_buffer = (double*)ldm_malloc(B_buffer_size * sizeof(double)); // 36KB
    double* B_rma_buffer = (double*)ldm_malloc(B_buffer_size * sizeof(double)); // 36KB

    double* tmp = NULL;

    double C_buf[block_m * block_n];
    // double* C_buf = (double*)ldm_malloc(block_m * block_n * sizeof(double));
    ...
    int A_row_start = _ROW * block_m;
    int A_col_start = ((_COL + _ROW) % 8) * block_k;

    int B_row_start = ((_ROW + _COL) % 8) * block_k;
    int B_col_start = _COL * block_n;

    int C_row_start = _ROW * block_m;
    int C_col_start = _COL * block_n;

    dma_rply = 0;
    // 1、DMA加载A的块到LDM  读取规则:（_ROW, _COL) 处的从核读取 （_ROW, (_COL + _ROW) % 8) 处的数据块
    CRTS_dma_iget_stride(A_dma_buffer, &para.A[A_row_start * para.lda + A_col_start],
        block_m * block_k * sizeof(double), block_k * sizeof(double), (para.lda - block_k) * sizeof(double), &dma_rply);
    // 2、DMA加载B的块到LDM  读取规则:（_ROW, _COL) 处的从核读取 （(_ROW + _COL) % 8, _COL) 处的数据块
    CRTS_dma_iget_stride(B_dma_buffer, &para.B[B_row_start * para.ldb + B_col_start],
        block_k * block_n * sizeof(double), block_n * sizeof(double), (para.ldb - block_n) * sizeof(double), &dma_rply);


    // if (_PEN== 0) printf("[row %d, col %d, num %d]: after ldm space: %dKB \n", _ROW, _COL, _PEN, CRTS_pldm_get_free_size()/1024);

    // if (_PEN== 0) printf("[block_m %d, block_n %d, block_k %d] \n", block_m, block_n, block_k);
    ...
    for (int i = 1; i <= 8; ++i) {
        rma_rply = 0;
        athread_ssync_array();
        // 【RMA读A右边的块A_dma_buffer到本从核的A_rma_buffer】
        //  读取规则:（_ROW, _COL) 处的从核读取 （_ROW, (_COL + 1 ) % 8)
        CRTS_rma_iget(A_rma_buffer, &rma_rply, block_m * block_k * sizeof(double),
                             _ROW * 8 + (_COL + 1 ) % 8, A_dma_buffer, &rma_rply);
        // 【RMA读B下边的块A_dma_buffer到本从核的B_rma_buffer】
        //  读取规则:（_ROW, _COL) 处的从核读取 （(_ROW + 1) % 8, _COL)
        CRTS_rma_iget(B_rma_buffer, &rma_rply, block_k * block_m * sizeof(double),
                             ((_ROW + 1) % 8) * 8 + _COL, B_dma_buffer, &rma_rply);

        // 计算dma-buffer里面的数据                
        for(int i = 0; i < block_m; ++i) {
            for(int j = 0; j < block_n; ++j) {
                for ( int k = 0; k < block_k; ++k) {
                    C_buf[i * block_n + j] += A_dma_buffer[i * block_k + k] *  B_dma_buffer[k * block_n + j];
                }
            }
        }

        CRTS_rma_wait_value(&rma_rply, 2);

        // 交换dma-rma 的buffer
        // 把rma里面的数据交给dma
        tmp = A_rma_buffer; 
        A_rma_buffer = A_dma_buffer;
        A_dma_buffer = tmp;
        tmp = B_rma_buffer; 
        B_rma_buffer = B_dma_buffer;
        B_dma_buffer = tmp;
        
    }

    CRTS_dma_put_stride(&para.C[C_row_start * para.ldc + C_col_start], C_buf, 
                        block_m * block_n * sizeof(double), block_n * sizeof(double), (para.ldc - block_n) * sizeof(double));

    ldm_free(A_dma_buffer, A_buffer_size * sizeof(double));
    ldm_free(A_rma_buffer, A_buffer_size * sizeof(double));
    ldm_free(B_dma_buffer, B_buffer_size * sizeof(double));
    ldm_free(B_rma_buffer, B_buffer_size * sizeof(double));
    // ldm_free(C_buf,        block_m * block_n * sizeof(double));
